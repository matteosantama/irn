...
theorem centralPath_norm_sq {μ : ℝ} (hμ : 0 < μ) {u : H X Y}
    (h : 𝓟.IsCentralPathPoint μ u) :
    ‖u‖ ^ 2 = (𝓟.ν : ℝ) + 1 := by
  obtain ⟨hC, hT⟩ := h
  have key : inner ℝ u (𝓟.T μ u) = (0 : ℝ) := by
