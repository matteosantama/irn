...
noncomputable def w0 (μ : ℝ) (hμ : 0 < μ) (u_k : H X Y)
    (hu_k : u_k ∈ 𝓢.C_interior) (z : H X Y) : H X Y :=
  (𝓢.hessian_plus_M μ hμ u_k hu_k).symm (𝓢.hessian_h μ u_k z)
...
noncomputable def quad_α (μ : ℝ) (hμ : 0 < μ) (u_k : H X Y)
    (hu_k : u_k ∈ 𝓢.C_interior) : ℝ :=
  𝓢.tau_proj (𝓢.w1 μ hμ u_k hu_k) -
    𝓢.Px_bilinform_clm (𝓢.w1 μ hμ u_k hu_k) (𝓢.w1 μ hμ u_k hu_k)

/-- `β = w_0^τ - 2 w_0^x⊤P w_1^x`. -/
noncomputable def quad_β (μ : ℝ) (hμ : 0 < μ) (u_k : H X Y)
    (hu_k : u_k ∈ 𝓢.C_interior) (z : H X Y) : ℝ :=
  𝓢.tau_proj (𝓢.w0 μ hμ u_k hu_k z) -
    2 * 𝓢.Px_bilinform_clm (𝓢.w0 μ hμ u_k hu_k z) (𝓢.w1 μ hμ u_k hu_k)

/-- `γ = -(w_0^x⊤P w_0^x + μ)`. -/
noncomputable def quad_γ (μ : ℝ) (hμ : 0 < μ) (u_k : H X Y)
    (hu_k : u_k ∈ 𝓢.C_interior) (z : H X Y) : ℝ :=
...
theorem exists_pos_root_quad (μ : ℝ) (hμ : 0 < μ) (u_k : H X Y)
    (hu_k : u_k ∈ 𝓢.C_interior) (z : H X Y) :
    ∃ θ : ℝ, 0 < θ ∧
      𝓢.quad_α μ hμ u_k hu_k * θ ^ 2 +
        𝓢.quad_β μ hμ u_k hu_k z * θ + 𝓢.quad_γ μ hμ u_k hu_k z = 0 ∧
      0 < 𝓢.tau_proj (𝓢.w0 μ hμ u_k hu_k z + θ • 𝓢.w1 μ hμ u_k hu_k) := by
  obtain ⟨u, hu_Cplus, θ, hθ_pos, h_eq, h_scalar⟩ :=
    𝓢.resolvent_exists μ u_k z hμ hu_k
  -- Step 1: u = w_0 + θ • w_1, from the augmented equation.
  have h_u_decomp : u = 𝓢.w0 μ hμ u_k hu_k z + θ • 𝓢.w1 μ hμ u_k hu_k := by
    have h_apply :
        (𝓢.hessian_plus_M μ hμ u_k hu_k : H X Y →L[ℝ] H X Y) u =
      ...
      exact hsum
    have h_u : u = (𝓢.hessian_plus_M μ hμ u_k hu_k).symm
                    (𝓢.hessian_h μ u_k z + θ • 𝓢.e_τ) := by
      rw [← h_apply]
      exact ((𝓢.hessian_plus_M μ hμ u_k hu_k).symm_apply_apply u).symm
    rw [h_u, map_add, map_smul]
    rfl
  -- Step 2: tau_proj (w_0 + θ • w_1) > 0 from u ∈ Cplus.
  have h_tau_pos : 0 < 𝓢.tau_proj
      (𝓢.w0 μ hμ u_k hu_k z + θ • 𝓢.w1 μ hμ u_k hu_k) := by
    rw [← h_u_decomp]
    exact 𝓢.tau_proj_pos hu_Cplus
  -- Step 3: Quadratic identity. Substitute u and expand.
  have h_quad : 𝓢.quad_α μ hμ u_k hu_k * θ ^ 2 +
                𝓢.quad_β μ hμ u_k hu_k z * θ + 𝓢.quad_γ μ hμ u_k hu_k z = 0 := by
    have h_scalar' :
        θ * 𝓢.tau_proj (𝓢.w0 μ hμ u_k hu_k z + θ • 𝓢.w1 μ hμ u_k hu_k) =
        𝓢.Px_bilinform_clm (𝓢.w0 μ hμ u_k hu_k z + θ • 𝓢.w1 μ hμ u_k hu_k)
          (𝓢.w0 μ hμ u_k hu_k z + θ • 𝓢.w1 μ hμ u_k hu_k) + μ := by
    ...
    have h_sym : 𝓢.Px_bilinform_clm (𝓢.w1 μ hμ u_k hu_k)
                    (𝓢.w0 μ hμ u_k hu_k z) =
                  𝓢.Px_bilinform_clm (𝓢.w0 μ hμ u_k hu_k z)
                    (𝓢.w1 μ hμ u_k hu_k) :=
...
theorem resolvent_closed_form
    (μ : ℝ) (hμ : 0 < μ) (u_k : H X Y) (hu_k : u_k ∈ 𝓢.C_interior)
    (z : H X Y) :
    ∃ u : H X Y, ∃ θ : ℝ,
      u ∈ 𝓢.Cplus ∧
      0 < θ ∧
  ...
  obtain ⟨θ, hθ_pos, h_quad, h_tau_pos⟩ :=
    𝓢.exists_pos_root_quad μ hμ u_k hu_k z
  refine ⟨𝓢.w0 μ hμ u_k hu_k z + θ • 𝓢.w1 μ hμ u_k hu_k, θ,
    ?_, hθ_pos, ?_, ?_⟩
  -- (1) u ∈ Cplus.
  · exact 𝓢.Cplus_of_tau_proj_pos h_tau_pos
  -- (2) (H_k + M) u = H_k z + θ • e_τ.
  · have h_app_w0 :
        (𝓢.hessian_h μ u_k + 𝓢.M_clm) (𝓢.w0 μ hμ u_k hu_k z) =
          𝓢.hessian_h μ u_k z := by
      have h_eq := 𝓢.hessian_plus_M_eq μ hμ u_k hu_k (𝓢.w0 μ hμ u_k hu_k z)
      have h_apply := 𝓢.hessian_plus_M_w0 μ hμ u_k hu_k z
      rw [ContinuousLinearMap.add_apply, ← h_eq]; exact h_apply
    have h_app_w1 :
        (𝓢.hessian_h μ u_k + 𝓢.M_clm) (𝓢.w1 μ hμ u_k hu_k) = 𝓢.e_τ := by
      have h_eq := 𝓢.hessian_plus_M_eq μ hμ u_k hu_k (𝓢.w1 μ hμ u_k hu_k)
      have h_apply := 𝓢.hessian_plus_M_w1 μ hμ u_k hu_k
      rw [ContinuousLinearMap.add_apply, ← h_eq]; exact h_apply
    calc (𝓢.hessian_h μ u_k + 𝓢.M_clm)
            (𝓢.w0 μ hμ u_k hu_k z + θ • 𝓢.w1 μ hμ u_k hu_k)
        = (𝓢.hessian_h μ u_k + 𝓢.M_clm) (𝓢.w0 μ hμ u_k hu_k z) +
            (𝓢.hessian_h μ u_k + 𝓢.M_clm)
              (θ • 𝓢.w1 μ hμ u_k hu_k) := by rw [map_add]
      _ = (𝓢.hessian_h μ u_k + 𝓢.M_clm) (𝓢.w0 μ hμ u_k hu_k z) +
            θ • (𝓢.hessian_h μ u_k + 𝓢.M_clm)
              (𝓢.w1 μ hμ u_k hu_k) := by rw [map_smul]
      _ = 𝓢.hessian_h μ u_k z + θ • 𝓢.e_τ := by rw [h_app_w0, h_app_w1]
  -- (3) θ · tau_proj u = B_P(u, u) + μ.
  · set T0 := 𝓢.tau_proj (𝓢.w0 μ hμ u_k hu_k z)
    set T1 := 𝓢.tau_proj (𝓢.w1 μ hμ u_k hu_k)
    set S0 := 𝓢.Px_bilinform_clm (𝓢.w0 μ hμ u_k hu_k z)
                                  (𝓢.w0 μ hμ u_k hu_k z)
    set S01 := 𝓢.Px_bilinform_clm (𝓢.w0 μ hμ u_k hu_k z)
                                  (𝓢.w1 μ hμ u_k hu_k)
    set S1 := 𝓢.Px_bilinform_clm (𝓢.w1 μ hμ u_k hu_k)
                                  (𝓢.w1 μ hμ u_k hu_k)
    have h_tau_u : 𝓢.tau_proj
        (𝓢.w0 μ hμ u_k hu_k z + θ • 𝓢.w1 μ hμ u_k hu_k) =
        T0 + θ * T1 := by
      rw [𝓢.tau_proj_add, 𝓢.tau_proj_smul]
    have h_Px_u :
