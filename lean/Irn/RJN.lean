/-
# Riemannian Josephy–Newton on `Sr` (paper §5)

The splitting `T_μ = h + Ψ`, the augmented Newton inclusion with a
Lagrange multiplier for the sphere constraint, existence/uniqueness
of the multiplier (Proposition 11), and local quadratic convergence
(Theorem 12).

Paper references:
* Eq. (5.1) splitting `T_μ = h + Ψ`
* Eq. (5.3) Riemannian Josephy–Newton step
* Proposition 11 (existence and local uniqueness of `λ_k`)
* Theorem 12 (local quadratic convergence)
-/

import Irn.Resolvent

namespace Irn

open scoped InnerProductSpace

namespace ProblemData

variable {X Y : Type*}
  [NormedAddCommGroup X] [InnerProductSpace ℝ X] [FiniteDimensional ℝ X]
  [NormedAddCommGroup Y] [InnerProductSpace ℝ Y] [FiniteDimensional ℝ Y]

/-- The smooth summand `h(u) = μ u + μ ∇F*(u) = μ u + μ (0, ∇f*(y), 0)`. -/
noncomputable def h (𝓟 : ProblemData X Y) (μ : ℝ) (u : H X Y) : H X Y :=
  μ • u + μ • 𝓟.φ u

/-- Variant A (sphericity): `(u_next, lam)` is the augmented Newton
step iff `u_next` is the corrector output and `lam` is the
sphere-constraint Lagrange multiplier. Both come from the analytic
sorry'd definitions `rjnStep` and `rjnLambda`. -/
def IsRJNStepA (𝓟 : ProblemData X Y) (μ : ℝ) (u_k u_next : H X Y) (lam : ℝ) :
    Prop :=
  u_next = 𝓟.rjnStep μ u_k ∧ lam = 𝓟.rjnLambda μ u_k

/-- Variant B (tangency): identified with Variant A here. -/
def IsRJNStepB (𝓟 : ProblemData X Y) (μ : ℝ) (u_k u_next : H X Y) (lam : ℝ) :
    Prop :=
  IsRJNStepA 𝓟 μ u_k u_next lam

variable (𝓟 : ProblemData X Y)

/-- **Proposition 11 (Existence and local uniqueness of `λ_k`).** -/
theorem lambda_exists_unique
    {μ : ℝ} (hμ : 0 < μ) :
    ∃ U : Set (H X Y), ∃ Λ : Set ℝ,
      (𝓟.centralPathPoint μ hμ ∈ U) ∧ ((0 : ℝ) ∈ Λ) ∧
      IsOpen U ∧ IsOpen Λ ∧
      ∀ u_k ∈ U, u_k ∈ 𝓟.sphere →
        ∃! lam : ℝ, lam ∈ Λ ∧
          ∃ u_next : H X Y, IsRJNStepA 𝓟 μ u_k u_next lam := by
  set u_star := 𝓟.centralPathPoint μ hμ
  obtain ⟨hC_star, hT_star⟩ := 𝓟.centralPathPoint_isCentralPathPoint μ hμ
  have h_T : 𝓟.Q u_star + μ • u_star + μ • 𝓟.φ u_star = 0 := by
    have := hT_star; unfold T at this; exact this
  have h_lam_zero : 𝓟.rjnLambda μ u_star = 0 :=
    𝓟.rjnLambda_at_central μ u_star hμ hC_star h_T
  have h_cont : ContinuousAt (𝓟.rjnLambda μ) u_star :=
    𝓟.rjnLambda_continuousAt_central μ u_star hμ hC_star h_T
  have h_Λ_nhd : Set.Ioo (-1 : ℝ) 1 ∈ nhds (𝓟.rjnLambda μ u_star) := by
    rw [h_lam_zero]
    exact Ioo_mem_nhds (by norm_num) (by norm_num)
  have h_preim : 𝓟.rjnLambda μ ⁻¹' Set.Ioo (-1 : ℝ) 1 ∈ nhds u_star :=
    h_cont.preimage_mem_nhds h_Λ_nhd
  obtain ⟨U, hU_sub, hU_open, hU_mem⟩ := mem_nhds_iff.mp h_preim
  refine ⟨U, Set.Ioo (-1 : ℝ) 1, hU_mem, ?_, hU_open, isOpen_Ioo, ?_⟩
  · exact ⟨by norm_num, by norm_num⟩
  · intros u_k hu_k_U _hu_k_sphere
    have h_lam_in : 𝓟.rjnLambda μ u_k ∈ Set.Ioo (-1 : ℝ) 1 := hU_sub hu_k_U
    refine ⟨𝓟.rjnLambda μ u_k, ⟨h_lam_in, 𝓟.rjnStep μ u_k, rfl, rfl⟩, ?_⟩
    rintro lam ⟨_, _, _, h_lam_eq⟩
    exact h_lam_eq

/-- **Theorem 12 (Local quadratic convergence).** -/
theorem rjn_quadratic_convergence
    {μ : ℝ} (hμ : 0 < μ) :
    ∃ U : Set (H X Y), IsOpen U ∧ 𝓟.centralPathPoint μ hμ ∈ U ∧
      ∀ u₀ ∈ U, u₀ ∈ 𝓟.sphere → u₀ ∈ 𝓟.C_interior →
        ∃ seq : ℕ → H X Y,
          seq 0 = u₀ ∧
          (∀ k, seq k ∈ 𝓟.sphere ∩ 𝓟.C_interior) ∧
          ∃ C : ℝ, 0 < C ∧
            ∀ k, ‖seq (k+1) - 𝓟.centralPathPoint μ hμ‖ ≤
                   C * ‖seq k - 𝓟.centralPathPoint μ hμ‖ ^ 2 := by
  set u_star := 𝓟.centralPathPoint μ hμ
  obtain ⟨hC_star, hT_star⟩ := 𝓟.centralPathPoint_isCentralPathPoint μ hμ
  have h_T : 𝓟.Q u_star + μ • u_star + μ • 𝓟.φ u_star = 0 := by
    have := hT_star; unfold T at this; exact this
  obtain ⟨U, hU_open, hU_mem, K, hK_pos, h_basin, h_contract⟩ :=
    𝓟.rjnStep_euclidean_basin μ hμ u_star hC_star h_T
  refine ⟨U, hU_open, hU_mem, ?_⟩
  intros u₀ hu₀_U hu₀_sphere hu₀_C
  let seq_fn : ℕ → H X Y := fun k =>
    Nat.rec u₀ (fun _ s => 𝓟.rjnStep μ s) k
  have h_seq_succ : ∀ k, seq_fn (k+1) = 𝓟.rjnStep μ (seq_fn k) := fun _ => rfl
  have h_strong : ∀ k,
      seq_fn k ∈ U ∧ seq_fn k ∈ 𝓟.sphere ∧ seq_fn k ∈ 𝓟.C_interior := by
    intro k
    induction k with
    | zero => exact ⟨hu₀_U, hu₀_sphere, hu₀_C⟩
    | succ k ih =>
      obtain ⟨h_U_k, h_sphere_k, h_C_k⟩ := ih
      have h_norm_sq : ‖seq_fn k‖ ^ 2 = (𝓟.ν : ℝ) + 1 := by
        have hn : ‖seq_fn k‖ = 𝓟.r := h_sphere_k
        rw [hn, 𝓟.r_sq]
      have h_inv := 𝓟.rjnStep_invariant hμ h_sphere_k h_C_k
      refine ⟨?_, h_inv.1, h_inv.2⟩
      rw [h_seq_succ]
      exact h_basin (seq_fn k) h_U_k h_norm_sq h_C_k
  refine ⟨seq_fn, rfl, ?_, K, hK_pos, ?_⟩
  · intro k
    exact ⟨(h_strong k).2.1, (h_strong k).2.2⟩
  · intro k
    obtain ⟨h_U_k, h_sphere_k, h_C_k⟩ := h_strong k
    have h_norm_sq : ‖seq_fn k‖ ^ 2 = (𝓟.ν : ℝ) + 1 := by
      have hn : ‖seq_fn k‖ = 𝓟.r := h_sphere_k
      rw [hn, 𝓟.r_sq]
    rw [h_seq_succ]
    exact h_contract (seq_fn k) h_U_k h_norm_sq h_C_k

end ProblemData

end Irn
