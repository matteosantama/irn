...
theorem proj_phi_on_sphere {u : H X Y}
    ...
    proj u (𝓟.φ u) = 𝓟.φ u + u := by
  unfold proj
  rw [𝓟.inner_u_phi u hC]
  have huu : ‖u‖ ^ 2 = (𝓟.ν : ℝ) + 1 := by
    have hnorm : ‖u‖ = 𝓟.r := hS
    rw [hnorm, 𝓟.r_sq]
  rw [huu]
  have hpos : (0 : ℝ) < (𝓟.ν : ℝ) + 1 := by positivity
...
theorem tildeT_transfer (μ σ : ℝ) (hμ : 0 < μ) (hσ : 0 < σ) {u : H X Y}
    (hS : u ∈ 𝓟.sphere) (hC : u ∈ 𝓟.C_interior) :
    𝓟.tildeT (σ * μ) u = 𝓟.tildeT μ u - ((1 - σ) * μ) • (𝓟.φ u + u) := by
  unfold tildeT
  rw [proj_of_orthogonal u _ (𝓟.tangent_T (mul_pos hσ hμ) hS hC),
      proj_of_orthogonal u _ (𝓟.tangent_T hμ hS hC)]
  unfold T
  module
...
noncomputable def R (u : H X Y) : ℝ := ‖𝓟.φ u + u‖

/-- The identity `μ R(u*(μ)) = ‖Q(u*(μ))‖`. -/
theorem mu_R_eq_Q_norm {μ : ℝ} (hμ : 0 < μ) :
    μ * 𝓟.R (𝓟.centralPathPoint μ hμ) =
      ‖𝓟.Q (𝓟.centralPathPoint μ hμ)‖ := by
  set u := 𝓟.centralPathPoint μ hμ
  obtain ⟨_, hT⟩ := 𝓟.centralPathPoint_isCentralPathPoint μ hμ
  unfold T at hT
  unfold R
  have hT' : 𝓟.Q u + (μ • u + μ • 𝓟.φ u) = 0 := by
    rw [← add_assoc]; exact hT
  have hQ : 𝓟.Q u = -(μ • (𝓟.φ u + u)) := by
    have h1 : 𝓟.Q u = -(μ • u + μ • 𝓟.φ u) := add_eq_zero_iff_eq_neg.mp hT'
    rw [h1, smul_add, add_comm (μ • 𝓟.φ u)]
  rw [hQ, norm_neg, norm_smul, Real.norm_eq_abs, abs_of_pos hμ]

/-- **Proposition 12 (Failure of Euclidean tolerances).** -/
theorem euclidean_failure
    {μ : ℝ} (hμ : 0 < μ) {σ : ℝ} (hσ : 0 < σ) (hσ' : σ ≤ 1) :
    ‖𝓟.tildeT (σ * μ) (𝓟.centralPathPoint μ hμ)‖ / (σ * μ) =
      ((1 - σ) / σ) * (‖𝓟.Q (𝓟.centralPathPoint μ hμ)‖ / μ) := by
  set u := 𝓟.centralPathPoint μ hμ
  obtain ⟨hC, hT⟩ := 𝓟.centralPathPoint_isCentralPathPoint μ hμ
  have hS : u ∈ 𝓟.sphere := by
    show ‖u‖ = 𝓟.r
    have h1 : ‖u‖ ^ 2 = (𝓟.ν : ℝ) + 1 :=
      𝓟.centralPath_norm_sq hμ ⟨hC, hT⟩
    unfold r
    rw [← Real.sqrt_sq (norm_nonneg u), h1]
  have h_tildeT_μ : 𝓟.tildeT μ u = 0 := by
    unfold tildeT
    rw [hT]
    simp [proj]
  have h_transfer := 𝓟.tildeT_transfer μ σ hμ hσ hS hC
  ...
  have hR : μ * 𝓟.R u = ‖𝓟.Q u‖ := 𝓟.mu_R_eq_Q_norm hμ
  unfold R at hR
  rw [show (1 - σ) * μ * ‖𝓟.φ u + u‖ = (1 - σ) * (μ * ‖𝓟.φ u + u‖)
        from by ring, hR]
...
noncomputable def delta (μ : ℝ) (u : H X Y) : ℝ :=
  𝓟.normWinv u (𝓟.tildeT μ u) / μ

/-- **Lemma 13 (Bounded transfer constant).** -/
theorem transfer_bound {u : H X Y}
    (hS : u ∈ 𝓟.sphere) (hC : u ∈ 𝓟.C_interior) :
    𝓟.normWinv u (𝓟.φ u + u) ≤ 2 * Real.sqrt ((𝓟.ν : ℝ) + 1) := by
  have h_tri : 𝓟.normWinv u (𝓟.φ u + u) ≤
      𝓟.normWinv u (𝓟.φ u) + 𝓟.normWinv u u :=
    𝓟.normWinv_triangle u (𝓟.φ u) u
  have h_phi : 𝓟.normWinv u (𝓟.φ u) ≤ Real.sqrt ((𝓟.ν : ℝ) + 1) :=
    𝓟.normWinv_phi_bound u hC
  have h_u : 𝓟.normWinv u u ≤ ‖u‖ := 𝓟.normWinv_le_norm u u
  have h_u_eq : ‖u‖ = Real.sqrt ((𝓟.ν : ℝ) + 1) := by
    have h_norm : ‖u‖ = 𝓟.r := hS
    unfold r at h_norm
    exact h_norm
  calc 𝓟.normWinv u (𝓟.φ u + u)
      ≤ 𝓟.normWinv u (𝓟.φ u) + 𝓟.normWinv u u := h_tri
    _ ≤ Real.sqrt ((𝓟.ν : ℝ) + 1) + ‖u‖ := by linarith
    _ = Real.sqrt ((𝓟.ν : ℝ) + 1) + Real.sqrt ((𝓟.ν : ℝ) + 1) := by rw [h_u_eq]
    _ = 2 * Real.sqrt ((𝓟.ν : ℝ) + 1) := by ring

/-- **Lemma 14 (Proximity transfer).** -/
theorem proximity_transfer
    {μ : ℝ} (hμ : 0 < μ) {σ : ℝ} (hσ : 0 < σ) (hσ' : σ ≤ 1)
    {u : H X Y} (hS : u ∈ 𝓟.sphere) (hC : u ∈ 𝓟.C_interior) :
    𝓟.delta (σ * μ) u ≤
      𝓟.delta μ u / σ +
        2 * (1 - σ) * Real.sqrt ((𝓟.ν : ℝ) + 1) / σ := by
  unfold delta
  rw [𝓟.tildeT_transfer μ σ hμ hσ hS hC]
  have h_1sub_μ_nn : 0 ≤ (1 - σ) * μ := mul_nonneg (by linarith) hμ.le
  have h_step1 :
      𝓟.normWinv u (𝓟.tildeT μ u - ((1 - σ) * μ) • (𝓟.φ u + u)) ≤
      𝓟.normWinv u (𝓟.tildeT μ u) +
        (1 - σ) * μ * 𝓟.normWinv u (𝓟.φ u + u) := by
    rw [sub_eq_add_neg, ← neg_smul]
    have h_tri := 𝓟.normWinv_triangle u (𝓟.tildeT μ u)
      ((-((1 - σ) * μ)) • (𝓟.φ u + u))
    rw [𝓟.normWinv_smul, abs_neg, abs_of_nonneg h_1sub_μ_nn] at h_tri
    exact h_tri
  have h_tb := 𝓟.transfer_bound hS hC
  have h_step2 :
      𝓟.normWinv u (𝓟.tildeT μ u - ((1 - σ) * μ) • (𝓟.φ u + u)) ≤
      𝓟.normWinv u (𝓟.tildeT μ u) +
        (1 - σ) * μ * (2 * Real.sqrt ((𝓟.ν : ℝ) + 1)) := by
  ...
  have h_rhs_eq :
      (𝓟.normWinv u (𝓟.tildeT μ u) +
            (1 - σ) * μ * (2 * Real.sqrt ((𝓟.ν : ℝ) + 1))) / (σ * μ) =
        𝓟.normWinv u (𝓟.tildeT μ u) / μ / σ +
          2 * (1 - σ) * Real.sqrt ((𝓟.ν : ℝ) + 1) / σ := by
    field_simp
  linarith
...
theorem outer_iteration_count
    ...
    {ε : ℝ} (hε : 0 < ε)
    {k : ℕ}
    (hk : (k : ℝ) ≥ Real.sqrt ((𝓟.ν : ℝ) + 1) / α * Real.log (1 / ε)) :
    (1 - α / Real.sqrt ((𝓟.ν : ℝ) + 1)) ^ k ≤ ε := by
  set s := Real.sqrt ((𝓟.ν : ℝ) + 1) with hs_def
  have hs_pos : 0 < s := 𝓟.r_pos
  have hs_ge_1 : 1 ≤ s := by
    have hν : 1 ≤ (𝓟.ν : ℝ) := by exact_mod_cast 𝓟.ν_pos
    calc (1 : ℝ) = Real.sqrt 1 := Real.sqrt_one.symm
      _ ≤ Real.sqrt ((𝓟.ν : ℝ) + 1) := Real.sqrt_le_sqrt (by linarith)
...
theorem rjnStep_quadratic_contraction :
    ∃ ρ_star K_star : ℝ, 0 < ρ_star ∧ ρ_star < 1 ∧ 1 ≤ K_star ∧
      ∀ {μ : ℝ}, 0 < μ → ∀ {u : H X Y}, u ∈ 𝓟.sphere → u ∈ 𝓟.C_interior →
        𝓟.delta μ u ≤ ρ_star →
          𝓟.delta μ (𝓟.rjnStep μ u) ≤ K_star * (𝓟.delta μ u) ^ 2 := by
  obtain ⟨ρ_star, K_star, hρ_pos, hρ_lt, hK_ge, h_field⟩ :=
    𝓟.rjnStep_delta_contraction
  refine ⟨ρ_star, K_star, hρ_pos, hρ_lt, hK_ge, ?_⟩
  intros μ hμ u hS hC h_δ
  have h_norm_sq : ‖u‖ ^ 2 = (𝓟.ν : ℝ) + 1 := by
    have hn : ‖u‖ = 𝓟.r := hS
    rw [hn, 𝓟.r_sq]
  have h_inv := 𝓟.rjnStep_invariant hμ hS hC
  have h_delta_u : 𝓟.delta μ u =
      𝓟.normWinv u (𝓟.Q u + μ • u + μ • 𝓟.φ u) / μ := by
    unfold delta tildeT
    rw [proj_of_orthogonal u _ (𝓟.tangent_T hμ hS hC)]
    rfl
  have h_delta_step : 𝓟.delta μ (𝓟.rjnStep μ u) =
      𝓟.normWinv (𝓟.rjnStep μ u)
        (𝓟.Q (𝓟.rjnStep μ u) + μ • (𝓟.rjnStep μ u) +
            μ • 𝓟.φ (𝓟.rjnStep μ u)) / μ := by
    unfold delta tildeT
    rw [proj_of_orthogonal _ _ (𝓟.tangent_T hμ h_inv.1 h_inv.2)]
    rfl
...
theorem hessian_quadratic_contraction :
    ∃ ρ_star : ℝ, ∃ K_star : ℝ,
      ...
      ∀ {μ : ℝ}, 0 < μ →
        ∀ {u : H X Y}, u ∈ 𝓟.sphere → u ∈ 𝓟.C_interior →
          𝓟.delta μ u ≤ ρ_star →
            ∀ {u_next : H X Y} {lam : ℝ}, IsRJNStepA 𝓟 μ u u_next lam →
              𝓟.delta μ u_next ≤ K_star * (𝓟.delta μ u) ^ 2 := by
  obtain ⟨ρ, K, hρ_pos, hρ_lt, hK_ge, h_contract⟩ :=
    𝓟.rjnStep_quadratic_contraction
...
theorem polynomial_complexity :
    ∃ α : ℝ, ∃ β : ℝ, ∃ N : ℕ,
      ...
      ∀ {μ₀ : ℝ}, 0 < μ₀ →
        ∀ {u₀ : H X Y}, u₀ ∈ 𝓟.sphere → u₀ ∈ 𝓟.C_interior →
          𝓟.delta μ₀ u₀ ≤ β →
            let σ := 1 - α / Real.sqrt ((𝓟.ν : ℝ) + 1)
            ∃ seq : ℕ → H X Y, seq 0 = u₀ ∧
              ∀ k, seq k ∈ 𝓟.sphere ∩ 𝓟.C_interior ∧
                   𝓟.delta (σ ^ k * μ₀) (seq k) ≤ β ∧
              ∀ ε : ℝ, 0 < ε →
                ∀ k, k ≥ ⌈Real.sqrt ((𝓟.ν : ℝ) + 1) / α
                          * Real.log (1 / ε)⌉₊ →
                  σ ^ k * μ₀ ≤ ε * μ₀ := by
  ...
  set s : ℝ := Real.sqrt ((𝓟.ν : ℝ) + 1) with hs_def
  have hs_pos : 0 < s := 𝓟.r_pos
  have hs_ge_1 : 1 ≤ s := by
    have hν : 1 ≤ (𝓟.ν : ℝ) := by exact_mod_cast 𝓟.ν_pos
    calc (1 : ℝ) = Real.sqrt 1 := Real.sqrt_one.symm
  ...
  let seq_fn : ℕ → H X Y := fun k =>
    Nat.rec (motive := fun _ => H X Y) u₀
      (fun n seq_n => 𝓟.rjnStep (σ^(n+1) * μ₀) seq_n) k
  have h_seq_0 : seq_fn 0 = u₀ := rfl
  have h_seq_succ : ∀ k, seq_fn (k+1) = 𝓟.rjnStep (σ^(k+1) * μ₀) (seq_fn k) :=
    fun _ => rfl
  have h_inv : ∀ k, seq_fn k ∈ 𝓟.sphere ∩ 𝓟.C_interior ∧
                    𝓟.delta (σ^k * μ₀) (seq_fn k) ≤ α := by
    intro k
    induction k with
    | zero =>
      refine ⟨⟨hS, hC⟩, ?_⟩
      show 𝓟.delta (σ^0 * μ₀) u₀ ≤ α
      simp
      exact h_δ_u₀
    | succ k ih =>
      obtain ⟨h_sC_k, h_δ_k⟩ := ih
      have h_S_k : seq_fn k ∈ 𝓟.sphere := h_sC_k.1
      have h_C_k : seq_fn k ∈ 𝓟.C_interior := h_sC_k.2
      have hμ_k_pos : 0 < σ^k * μ₀ := by positivity
      have hμ_succ_pos : 0 < σ^(k+1) * μ₀ := by positivity
      have h_pt := 𝓟.proximity_transfer hμ_k_pos hσ_pos hσ_le_1 h_S_k h_C_k
      have h_pow_succ_eq : σ * (σ^k * μ₀) = σ^(k+1) * μ₀ := by ring
      rw [h_pow_succ_eq] at h_pt
      have h_pre_contract : 𝓟.delta (σ^(k+1) * μ₀) (seq_fn k) ≤ 6 * α := by
        have h_step : 𝓟.delta (σ^(k+1) * μ₀) (seq_fn k) ≤ 3 * α / σ := by
          calc 𝓟.delta (σ^(k+1) * μ₀) (seq_fn k)
              ≤ 𝓟.delta (σ^k * μ₀) (seq_fn k) / σ +
                  2 * (1 - σ) * s / σ := h_pt
            _ ≤ α / σ + 2 * (α / s) * s / σ := by
                apply add_le_add
        ...
        calc 𝓟.delta (σ^(k+1) * μ₀) (seq_fn k) ≤ 3 * α / σ := h_step
          _ ≤ 6 * α := by
      ...
      have h_in_basin : 𝓟.delta (σ^(k+1) * μ₀) (seq_fn k) ≤ ρ_star :=
        le_trans h_pre_contract h_6α_le_ρ
      have h_invariant := 𝓟.rjnStep_invariant hμ_succ_pos h_S_k h_C_k
      refine ⟨h_invariant, ?_⟩
      rw [h_seq_succ]
      have h_contract' := h_contract hμ_succ_pos h_S_k h_C_k h_in_basin
      have h_delta_nn : 0 ≤ 𝓟.delta (σ^(k+1) * μ₀) (seq_fn k) := by
        unfold delta
        apply div_nonneg
        · exact 𝓟.normWinv_nonneg _ _
        · exact hμ_succ_pos.le
      calc 𝓟.delta (σ^(k+1) * μ₀) (𝓟.rjnStep (σ^(k+1) * μ₀) (seq_fn k))
          ≤ K_star * (𝓟.delta (σ^(k+1) * μ₀) (seq_fn k)) ^ 2 := h_contract'
        _ ≤ K_star * (6 * α) ^ 2 := by
  ...
  have h_oic := 𝓟.outer_iteration_count hα_pos hα_lt_1 hε hk_iter_real
